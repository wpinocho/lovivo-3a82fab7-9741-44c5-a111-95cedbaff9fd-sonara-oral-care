...
function PaymentRequestInner({
  ...
  const { currencyCode, shippingCoverageV2 } = useSettings()
  ...
  // Placeholder shipping option — the real rate is fetched from the backend
  // (`shipping-rates`) once the wallet provides the shipping address.
  ...
  useEffect(() => {
    ...
    const onShippingAddressChange = async (ev: any) => {
      try {
        ...
        const shippingOptions = data.rates.map((r: any) => ({
          id: String(r.id),
          label: r.name || 'Envío',
          detail: r.delivery_estimate ? String(r.delivery_estimate) : '',
          amount: Math.round(Number(r.amount || 0) * 100),
        }))
        const firstShip = shippingOptions[0].amount
